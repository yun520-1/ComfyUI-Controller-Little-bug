...
def create_workflow(prompt, negative, width=1024, height=512, seed=None):
    """创建工作流 - 使用 DualCLIPLoaderGGUF"""
    if seed is None:
        seed = int(time.time() * 1000) % 1000000
    
    # 使用 DualCLIPLoaderGGUF 加载正确的 CLIP 配置
    workflow = {
        # 1. UNet 加载
        "1": {
        ...
        # 2. 使用 DualCLIPLoaderGGUF (Z-Image-Turbo 需要)
        "2": {
            "class_type": "DualCLIPLoaderGGUF",
            "inputs": {
                "clip_name1": "ltx-2-19b-dev_embeddings_connectors.safetensors",
                "clip_name2": "ltx-2-19b-dev_embeddings_connectors.safetensors",
                "type": "ltxv"
            }
        },
