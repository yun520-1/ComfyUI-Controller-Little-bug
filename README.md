...
```
[上传小说文件：《斗破苍穹.txt》]
帮我把这本小说的每个章节生成一张插图，尺寸 1024*1024，风格是玄幻修仙
```
...
| 场景 | 指令 |
|------|------|
| 新闻配图 | 「帮我生成 10 张图片，基于最新新闻，尺寸 1024*512」 |
| 小说插图 | [上传小说]「帮我把每个章节生成插图，玄幻风格」 |
| 提示词批量 | [上传 prompts.txt]「用这个列表生成图片」 |
| 指定工作流 | [上传文件 + workflow.json]「用这个工作流生成」 |
...
📊 生成结果汇总
═══════════════════════════════════════════
✅ 成功：10/10  💾 保存到：~/Downloads/comfyui_organized/news/
```
...
| 问题 | 解决方案 |
|------|---------|
| 生成失败 | 检查 ComfyUI 是否运行 |
| 模型缺失 | 自动下载或手动放置到 models/ |
| 显存不足 | 降低尺寸或分批生成 |
| 需要中断 | 说「停止生成」 |
...
或者直接说：「帮我生成 10 张图片，基于最新新闻，尺寸 1024*512」
