...
import json, uuid, time, requests, sys, webbrowser, random
...
# 导入小虫子优化器
try:
    from little_bug_optimizer import LittleBugOptimizer
    bug_optimizer = LittleBugOptimizer()
    print("✅ 小虫子优化器已加载")
except:
    bug_optimizer = None
    print("⚠️ 小虫子优化器未加载，使用基础模式")

COMFYUI_SERVER = "127.0.0.1:8188"
CONTROLLER_PORT = 8190
OUTPUT_DIR = Path.home() / "Downloads" / "comfyui_auto_images"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 23 种尺寸选项
SIZE_OPTIONS = [
    ("512x512", 512, 512), ("768x768", 768, 768), ("1024x1024", 1024, 1024), ("1536x1536", 1536, 1536),
    ("512x384", 512, 384), ("768x512", 768, 512), ("1024x512", 1024, 512), ("1280x720", 1280, 720),
    ("1536x864", 1536, 864), ("1920x1080", 1920, 1080), ("2048x1024", 2048, 1024),
    ("384x512", 384, 512), ("512x768", 512, 768), ("512x1024", 512, 1024), ("720x1280", 720, 1280),
    ("864x1536", 864, 1536), ("1080x1920", 1080, 1920), ("1024x2048", 1024, 2048),
    ("2560x1080", 2560, 1080), ("3440x1440", 3440, 1440),
    ("1080x1080", 1080, 1080), ("1080x1350", 1080, 1350), ("1200x630", 1200, 630),
]

# 提示词变体模板
VARIATION_TEMPLATES = {
    "style": [
        "realistic photography", "digital art", "oil painting style", "watercolor style",
        "anime style", "comic book style", "cinematic", "3D render", "concept art",
        "impressionist style", "cyberpunk style", "steampunk style", "fantasy art"
    ],
    "lighting": [
        "golden hour lighting", "soft natural light", "dramatic lighting", "studio lighting",
        "neon lights", "cinematic lighting", "volumetric lighting", "rim lighting",
        "backlit", "ambient light", "warm lighting", "cool lighting"
    ],
    "atmosphere": [
        "peaceful atmosphere", "dynamic atmosphere", "mysterious atmosphere", "energetic vibe",
        "romantic atmosphere", "epic atmosphere", "serene mood", "dramatic mood",
        "cheerful mood", "melancholic mood", "hopeful atmosphere", "intense atmosphere"
    ],
    "quality": [
        "ultra high quality", "masterpiece", "award winning", "professional grade",
        "highly detailed", "photorealistic", "stunning visuals", "exceptional quality",
        "pristine quality", "flawless detail", "crystal clear", "premium quality"
    ],
    "composition": [
        "rule of thirds", "centered composition", "wide angle view", "close-up shot",
        "panoramic view", "bird's eye view", "low angle shot", "dynamic angle",
        "symmetrical composition", "leading lines", "depth of field", "bokeh background"
    ]
}
...
class NewsFetcher:
    ...
    def fetch_latest_news():
        news_topics = [
            {"topic": "两会 2026", "prompt": "两会召开，人民大会堂，代表们讨论国家发展，庄严隆重"},
            {"topic": "汪峰演唱会", "prompt": "汪峰演唱会现场，舞台灯光璀璨，观众热情高涨，音乐氛围"},
            {"topic": "海洋经济", "prompt": "海洋经济论坛，蓝色海洋背景，渔船和港口，经济发展"},
            {"topic": "西湖马拉松", "prompt": "西湖马拉松比赛，选手奔跑在西湖边，风景优美，运动活力"},
    ...
    def search_web(query):
        try:
            result = subprocess.run(['curl', '-s', '-m', '5', f'https://cn.bing.com/search?q={query}'], capture_output=True, timeout=10)
            if result.returncode == 0:
                return {"success": True, "query": query, "found": True}
        except: pass
        return {"success": False, "query": query, "found": False}
...
class PromptVariator:
    ...
    def generate_variations(base_prompt, count=5):
        ...
        for i in range(count):
            ...
            lighting = random.choice(VARIATION_TEMPLATES["lighting"])
...
HTML = """<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>ComfyUI 智能变体控制器</title>
<style>
...
.queue-item{background:#f8f9fa;padding:15px;margin-bottom:10px;border-radius:8px;border-left:4px solid #667eea;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap}
...
.queue-item-prompt{font-size:0.85em;color:#666;margin-top:5px;font-style:italic}
.log-area{background:#1e1e1e;color:#d4d4d4;padding:15px;border-radius:8px;font-family:monospace;font-size:0.85em;max-height:300px;overflow-y:auto}
...
.variant-tag{display:inline-block;padding:2px 6px;border-radius:4px;font-size:0.75em;margin:1px;background:#fff3cd;color:#856404}
.info-box{background:#e3f2fd;padding:12px;border-radius:8px;margin-bottom:15px;border-left:4px solid #2196f3}
</style></head><body>
<div class="container">
<h1>🎨 ComfyUI 智能变体控制器</h1>
<div class="info-box">💡 <strong>智能变体模式：</strong>每个任务自动生成不同提示词，基于新闻/搜索关键词智能变化，确保每张图都独特！</div>
...
<div class="row"><div class="form-group"><label>尺寸 (23 种)</label><select id="size"></select></div>
<div class="form-group"><label>数量</label><input type="number" id="count" min="1" max="20" value="5"></div></div>
<div class="form-group"><label>任务间隔 (秒)</label><input type="number" id="interval" min="0" max="300" value="20"></div>
<div class="form-group"><label><input type="checkbox" id="enable-variants" checked> 启用智能变体（每个任务不同提示词）</label></div>
</div>
...
<button class="btn btn-small" onclick="useNews()">📰 使用新闻（自动变体）</button>
</div>
</div>
...
<div class="form-group"><label>基础提示词</label><textarea id="prompt" placeholder="输入基础提示词，或从上方选择预设/新闻..."></textarea></div>
...
<div class="card"><button type="button" class="btn" id="btn" onclick="start()">🚀 开始生成（智能变体）</button></div>
...
const SIZES=""" + json.dumps([[s[0],s[0]] for s in SIZE_OPTIONS]) + """;
sizes=SIZES;
...
document.getElementById('news-box').innerHTML=newsList.map((n,i)=>`<div class="news-item" onclick="useNewsSingle(${i})">📰 ${n.topic}${n.prompt?' - '+n.prompt.substring(0,30)+'...':''}</div>`).join('');}
catch(e){document.getElementById('news-box').innerHTML='<div class="news-item">❌ 获取失败</div>';}}

function useNewsSingle(i){if(newsList[i]){document.getElementById('prompt').value=newsList[i].prompt||newsList[i].topic;log('已使用新闻：'+newsList[i].topic,'info');}}
function useNews(){if(newsList.length===0){log('请先刷新新闻','warning');return;}document.getElementById('prompt').value=newsList.map(n=>n.prompt||n.topic).join('; ');log('已使用所有新闻（将自动变体）','info');}
...
if(d.success)log('搜索完成，将基于：'+q,'success');else log('搜索失败','error');}
...
q.innerHTML=tasks.map((t,i)=>`<li class="queue-item ${t.status}"><div class="queue-item-info"><strong>${i+1}. ${t.title}</strong><div class="queue-item-prompt">${t.variant_info||t.prompt.substring(0,80)}...</div></div><span>${t.status==='pending'?'⏳等待':t.status==='running'?'▶️运行':t.status==='completed'?'✅完成':'❌失败'}</span></li>`).join('');}
...
const data={mode:document.getElementById('mode').value,model:document.getElementById('model').value,size:document.getElementById('size').value,count:+document.getElementById('count').value,interval:+document.getElementById('interval').value,prompt:document.getElementById('prompt').value,negative:document.getElementById('negative').value,extra:document.getElementById('extra').value,enable_variants:document.getElementById('enable-variants').checked,search_query:document.getElementById('search-query').value};
...
log('开始：'+data.count+' 个变体 '+data.mode+', 尺寸 '+data.size,'info');
if(data.enable_variants)log('✅ 智能变体已启用：每个任务将使用不同提示词','success');
...
if(res.success){tasks=res.tasks;updateQueue();log('已创建 '+tasks.length+' 个变体任务','success');
const poll=setInterval(async()=>{await update();if(!controller_running){clearInterval(poll);running=false;document.getElementById('btn').disabled=false;document.getElementById('btn').textContent='🚀 开始生成（智能变体）';log('所有任务完成','success');}},2000);}
        ...
        if self.path == '/api/start':
            ...
            if enable_variants:
                ...
                if len(base_prompts) > 1:
                    ...
                    # 单个基础提示词，生成变体
                    if search_query:
                        variations = variator.generate_from_search(search_query, data['count'])
                    elif "新闻" in base_prompt or "news" in base_prompt.lower():
                        news_list = NewsFetcher.fetch_latest_news()
                        variations = variator.generate_from_news(news_list, data['count'])
                    else:
                        variations = variator.generate_variations(base_prompt, data['count'])
            else:
                # 不使用变体，所有任务相同提示词
            ...
            for i, var in enumerate(variations[:data['count']]):
                ...
                task = {
                    ...
                    "variant_id": var.get('variant_id', i+1),
...
def run_tasks(interval):
    ...
    for i, task in enumerate(controller.tasks):
        task['status'] = 'running'
        print(f"[{i+1}/{len(controller.tasks)}] {task['title']} - {task.get('variant_info', '')}")
...
def main():
    print("="*60)
    print("🎨 ComfyUI 智能变体控制器")
    print("="*60)
    print(f"\n✅ ComfyUI: {COMFYUI_SERVER}")
    print(f"📦 可用模型：{len(manager.available_models['unet'])} UNet")
    print(f"📐 尺寸选项：{len(SIZE_OPTIONS)} 种")
    print(f"🎭 变体模板：{len(VARIATION_TEMPLATES)} 类")
